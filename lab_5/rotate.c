...
//första 1/4 kolumn
  ...
  int x,y;
  // Useful variables for creating read sequence of pixels (use if you like)
  int i,j,k,l;         
...
for(column = 0; column < nrOfColumns; column++){
 // Initial solution: Copy pixels from left to right, line by line
  ...
  do {
    x = (FB_XSIZE/nrOfColumns)*column;   // start to the left of the line
    do {
      ...
      x = x + 1;
    } while (x < (FB_XSIZE/nrOfColumns)*(column+1));    // Do this until line completed
    y = y + 1;
