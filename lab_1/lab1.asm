...
main:
	...
	;F4240
	mov r12,#(0xF4240 & 0xffff) ; Timern för 1 sek
	movt r12,#(0xF4240 >> 16)	;Fyll r12 med 16 000 000
...
activatealarm:
	mov r9, #0x1 ; r9 indikerar att alarmet är aktiverat
...
;
; Funktion: T ̈ander röd lysdiod (bit 3 = 0, bit 2 = 0, bit 1 = 1)
activatealarmOffLamp:
	mov r0, #0x0 ;Laddar in 0000 0000. För att skapa rött
	mov r1,#(GPIOF_GPIODATA & 0xffff) ;laddar in halva adressen
	movt r1,#(GPIOF_GPIODATA >> 16); laddar in andra halvan av adressen
	strb r0, [r1] ; Lägger koden för färgen röd i r
	b getkey
...
; Funktion: T ̈ander röd lysdiod (bit 3 = 0, bit 2 = 0, bit 1 = 1)
activatealarmOnLamp:
	mov r0, #0x2 ;Laddar in 0000 0000. För att skapa rött
	mov r1,#(GPIOF_GPIODATA & 0xffff) ;laddar in halva adressen
	movt r1,#(GPIOF_GPIODATA >> 16); laddar in andra halvan av adressen
	strb r0, [r1] ; Lägger koden för färgen röd i r
	b getkey
...
toggleLamp:
	...
	mov r12,#(0xF4240 & 0xffff) ;resetar
	movt r12,#(0xF4240 >> 16)	;Fyll r12 med 16 000 000

	mov r11,#0x0; Så att den inte stannar i loopen

	mov r8,#(GPIOF_GPIODATA & 0xffff) ;laddar in inputkoden addressen
	movt r8,#(GPIOF_GPIODATA >> 16)

	ldrb r7,[r8] ; r7 = aktuell lampa

	;cmp r9, #0x0
	;beq turnLightGreen;

	cmp r7, #0x13
	beq activatealarmOffLamp;

	cmp r7, #0x11
	beq activatealarmOnLamp;
	b getkey
...
deactivatealarm:
	mov r9, #0x0 ; r9 indikerar att alarmet är aktiverat
